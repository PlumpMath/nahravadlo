...
namespace Nahravadlo
{
	internal class Channels
	{
		...
		public Channel[] getChannels()
		{
			...
			Channel[] ret = new Channel[nodes.Count];
		...
		public Channel[] loadChannelsFromFile(string filename)
		{
			...
			try
			{
				...
				XmlNamespaceManager ns = new XmlNamespaceManager(imp.NameTable);
				...
				for (int i = 0; i < nodes.Count; i++)
				{
					...
					uri = nodes[i].SelectSingleNode("xspf:location", ns).InnerText;
					name = uri;
