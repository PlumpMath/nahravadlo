...
namespace Nahravadlo
{
	...
	public class Job : IDisposable
	{
		...
		internal Job(Task task, TaskFolder folder, JobVersion version, string vlcFilename, string workingDirectory)
		{
			...
			{
				var trigger = (TimeTrigger) triggers[0];
		...
		public string Name
		{
			get
			{
				...
				if (version == JobVersion.V2)
					return name;
				if (name.StartsWith(Schedules.NEW_TASK_PREFIX))
					return name.Substring(Schedules.NEW_TASK_PREFIX.Length);
				if (name.StartsWith(Schedules.OLD_TASK_PREFIX))
					return name.Substring(Schedules.OLD_TASK_PREFIX.Length);
				return String.Empty;
			}
			set
			{
		...
		public void Save(String userName, String password)
		{
			...
			var args = UseMPEGTS ? string.Format("{0} :demux=dump :demuxdump-file=\"{1}\"", Uri, Filename) : string.Format("{0} :sout=#duplicate{{dst=std{{access=file,mux=ps,dst=\"{1}\"}}}}", Uri, Filename);
			...
			if (definition.Actions.Count > 0 && version == JobVersion.V2)
			{
				...
				{
					var action = (ExecAction) definition.Actions[0];
			...
			if (Filename.IndexOf('\\') != -1)
				Directory.CreateDirectory(Path.GetDirectoryName(Path.Combine(workingDirectory, Filename)));

			//ulozime ulohu
			var logonType = String.IsNullOrEmpty(password) ? TaskLogonType.InteractiveToken : TaskLogonType.Password;
			if (version == JobVersion.V2)
		...
		public void Terminate()
		{
			...
			//V2 nesmaze ulohu, musime to udelat rucne
			if (version == JobVersion.V2 && definition.Settings.DeleteExpiredTaskAfter > TimeSpan.Zero)
